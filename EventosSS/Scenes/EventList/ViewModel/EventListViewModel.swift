            ...
            DispatchQueue.main.async {
                if let result = result {
                    result.forEach { eventDAO in
                        ...
                        let event = Event(id: Int(eventDAO.id) ?? 0, title: eventDAO.title, date: date, description: eventDAO.description, imageURL: eventDAO.imageURL, people: people, price: eventDAO.price)
                        events.append(event)
                    }
